...
func (m *imageManager) EnsureImageExists(pod *v1.Pod, container *v1.Container, pullSecrets []v1.Secret, podSandboxConfig *runtimeapi.PodSandboxConfig) (string, string, error) {
	...
	if container.UseIPFS {
		...
		if _, err := os.Stat(downloadPath + container.IPFSHash); os.IsNotExist(err) {
			cmd0 := []string{ "runuser", "mkunal", "-c", "\"/usr/local/bin/ipfs pin add "+ container.IPFSHash+ "\""}
			_, msg0, err0 :=RunCmd(cmd0[0], cmd0[1:]...)
			if err0 != nil {
				return "", msg0, ErrImageInspect
			}
			cmd1 := []string{"runuser", "mkunal", "-c", "\"/usr/local/bin/ipfs get "+container.IPFSHash + " -o "+downloadPath+"\""}
